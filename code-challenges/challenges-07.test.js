...
const replaceZeros = (string) => {
  // Solution code here...
  const zeroRegex=/0/g;
  string=string.replace(zeroRegex,'zero');
  return string;
}
...
const addOne = (arr) => {
  // Solution code here...
  const addOne=arr.map((number,index)=>{
    return number + 1;
  });
  return addOne;
}
...
const addQuestion = (arr) => {
  // Solution code here...
  const addQuestionMark=arr.map((strings,index)=>{
    return strings + '?';
  })
...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i = 0; i< arr.length; i++) {
    MathPowArry.push(Math.pow(2,arr[i]));
      }
      return MathPowArry;
};
...
const forEachTwoToThe = (arr) => {
  ...
  const MathPowArry=[];
arr.forEach(element =>{
  MathPowArry.push(Math.pow(2,element));
...
const mapTwoToThe = (arr) => {
  // Solution code here...
  const mathPowMap=arr.map((number,index)=>{
