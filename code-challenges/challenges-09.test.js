...
function transformToLis(obj){
  // Solution code here...
  const arrayHtml=Object.keys(obj).map((key)=> `<li>${key}: ${obj[key]}</li>`);
...
const addValues = (arr) => {
  // Solution code here...
  const addValue= arr.reduce((number,value)=>{
    return number + value;
  },0);
  return addValue;
  console.log(arr);
...
const addPurchases = (arr) => {
  // Solution code here...
  const pureshed=arr.reduce((purshes,item)=>{
    purshes = purshes+item.purchasePrice;
    return purshes;
  },0);
  return pureshed;
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  const totalElement=arr.reduce((length)=>{
    length= length+1;
    return length;
  })
  return totalElement;
...
const returnNames = (arr) => {
  // Solution code here...
  const charName=arr.reduce((name,item)=>{
    name.push(item.name);
    return name;
  },[])
...
const reversedString = (str) => {
  ...
  const reservedStr=splitString.reduce((string,item)=>{
    return item +string;
  },'')
  return reservedStr;
};
