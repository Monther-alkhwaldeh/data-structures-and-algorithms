...
const getNames = (arr) => {
  // Solution code here...
  const arrName = arr.map(element => element.name);
  const result =[];
  arrName.forEach(element=> result.push(element.split('').reverse().join('')));
  return result;
};
...
const count = (target, input) => {
  ...
  const numArray = [];
  input.forEach(arr=>{
    numArray.push(arr.filter(element => element === target))
  })
  const result = numArray.toString().split(',').join('').length;
  return result;
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  input.forEach(element=>{
    divArr.push(element.filter(number=> typeof number== 'number'&& number %5===0).map(number=>Math.pow(2,number)))
  })
  return divArr;
};
...
let findMaleAndFemale = (data) => {
  ...
  const result = findArr.map(element=> element['name']).join(' and ')
  return result;
};
