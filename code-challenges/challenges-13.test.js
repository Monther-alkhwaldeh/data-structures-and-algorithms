...
const firstLetters = (arr) => {
  // Solution code here...
  let firstLetters = arr.map(s => s.charAt(0));
  return firstLetters;
...
const findHappiness = (arr) => {
  // Solution code here...
  const happy= arr.filter(e => e.includes(':)'));
 return happy;
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
   ...
   const phone= arr.map(e => e.replace(/[() -]/g, ''));
      return phone;
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  const oddChar= str.split('').filter((a, b) =>b % 2 !== 0 ? a : 0).join('');
  return oddChar;
};
...
const allHappy = (arr) => {
  // Solution code here...
  const boolean= arr.every(e =>e.includes(':)'));
  return boolean;
};
